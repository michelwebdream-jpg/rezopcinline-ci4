...
if ($errno !== 0 || $httpCode >= 400 || !is_string($response)) {
    error_log('envoi_mail_historique.php relay error: ' . $errno . ' ' . $error . ' url=' . $endpointUrl . ' code=' . $httpCode);
    echo 'return_txt=-1';
    exit;
}
...
error_log('envoi_mail_historique.php unexpected relay response: ' . substr($response, 0, 300));
echo 'return_txt=-1';
