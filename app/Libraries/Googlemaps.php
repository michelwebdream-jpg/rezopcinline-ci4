...
';*/
    ...
    ';
		...
		if ($this->minifyJS) {
			// CI4: Charger la library Jsmin
			$jsmin = new \App\Libraries\Jsmin();
			$this->output_js_contents = $jsmin->min($this->output_js_contents);
		}
	...
	function get_lat_long_from_address($address, $attempts = 0)
	{
		...
		if ($this->geocodeCaching) { // if caching of geocode requests is activated
			...
			// CI4: Utiliser le service Database
			$db = \Config\Database::connect();
			$query = $db->table('geocoding')
